...
public final class ByteConversor {
  ...
  private static final BiFunction<String, Integer, String> FILE_SIZE_TO_UNITS =
      (value, unit) -> new StringBuilder(value).append(" ")
          .append(FileConstants.getByteUnits()[unit]).toString();

  public static final BiPredicate<Long, Long> MIN_AVAILABLE_SIZE =
      (value, minAvailable) -> value.longValue() >= minAvailable.longValue();

  public static String readableFileSize(long size) {
    try {
      if (size <= NumericConstants.ZERO) {
        return FILE_SIZE_TO_UNITS.apply(String.valueOf(NumericConstants.ZERO),
