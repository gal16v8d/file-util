...
public final class FileUtil {
     ...
     * @param route
     * @param minDirSize
     *            minimum size for allow store.
     * @return
     */
    public static boolean checkAvailableSpaceOnDir(String route, Long minDirSize) {
        try {
            return ByteConversor.getMinAvailableSize(generateFileFromRoute(route).getFreeSpace(), minDirSize);
        } catch (Exception e) {
            throw new TechnicalException(e);
        }
    }
    ...
    public static List<File> getFilesSortedByLastModification(String route) {
        File f = generateFileFromRoute(route);
        Comparator<File> fileComparator = (File p1, File p2) -> Long.compare(p1.lastModified(), p2.lastModified());
    ...
    public static File getLastModifiedFile(String ruta) {
        try {
            ...
            return !filesOnDir.isEmpty() ? filesOnDir.get(filesOnDir.size() - NumericConstants.ONE) : null;
        } catch (Exception e) {
            throw new TechnicalException(e);
        }
    }
    ...
    public static boolean unzipFile(String inputZip, String outDir, int byteBuffer) {
        ...
        try (FileInputStream fis = new FileInputStream(inputZip); ZipInputStream zis = new ZipInputStream(fis);) {
            ...
            while ((zipFile = zis.getNextEntry()) != null) {
                ...
                byte[] buffer = new byte[byteBuffer];
                int read;
                while ((read = zis.read(buffer, NumericConstants.ZERO, buffer.length)) != NumericConstants.ZERO) {
